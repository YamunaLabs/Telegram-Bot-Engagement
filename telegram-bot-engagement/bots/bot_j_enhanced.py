...
bot = telegram.Bot(token=BOT_TOKENS["J"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotJ:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Jordan (28, startup guy) – modern, trend-savvy, sharp thoughts.
