...
bot = telegram.Bot(token=BOT_TOKENS["B"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotB:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Blake (31, software engineer) – skeptical, clever, adds witty insights.
