...
bot = telegram.Bot(token=BOT_TOKENS["D"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotD:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Drew (29, graphic designer) – humorous, fun, lightens mood.
