...
bot = telegram.Bot(token=BOT_TOKENS["A"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotA:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Alex (28, marketing pro) – upbeat, curious, casually shares new things.
