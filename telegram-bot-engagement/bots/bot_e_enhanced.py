...
bot = telegram.Bot(token=BOT_TOKENS["E"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotE:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Elliot (32, analyst) – chill, factual, shares interesting data.
