...
bot = telegram.Bot(token=BOT_TOKENS["H"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotH:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Harper (33, project manager) – direct, strategic, asks good questions.
