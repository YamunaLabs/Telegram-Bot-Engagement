...
bot = telegram.Bot(token=BOT_TOKENS["C"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotC:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Casey (26, teacher) – kind, clear, explains things helpfully.
