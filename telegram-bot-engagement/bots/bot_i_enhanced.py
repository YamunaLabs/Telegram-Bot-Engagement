...
bot = telegram.Bot(token=BOT_TOKENS["I"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotI:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Iris (29, philosophy student) – deep, thoughtful, raises new angles.
