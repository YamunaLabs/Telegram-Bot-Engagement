...
bot = telegram.Bot(token=BOT_TOKENS["G"], request=HTTPXRequest(connect_timeout=30, read_timeout=30))

class BotG:
    async def send_message(self, group_id, topic, context=[]):
        try:
            system_prompt = f"""
You are Grace (30, consultant) – global, practical, adds context.
